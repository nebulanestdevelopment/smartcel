...
<form id="repuestosInvForm">
          <input type="hidden" name="idAccesorioAutoInv" id="idAccesorioAutoInv">
...
<form id="ClienteForm">
          <input type="hidden" name="idAccesorioAuto" id="idAccesorioAuto">
...
function duplicarProducto(){
  var dataForm = {method:'duplicar-producto',
                        id:parseInt($("#idAccesorioAuto").val().trim())
                        };
        var request;        
        if (request) {
            request.abort();
        ...
        request.done(function (data, textStatus, jqXHR){
          toastr.success(data.msg, 'Exitoso', {timeOut: 3000});
          setTimeout(function(){window.location.href= './index.php?view=producto/accesorios-autos-edit&id='+data.id ;},2500);
...
function actualizarInventario(){
  ...
  var dataForm = {method:'update-inventario',
                        id:parseInt($("#idAccesorioAutoInv").val().trim()),
...
$("#ClienteForm").submit(function(event) {
  ...
  formData.append("id", parseInt($("#idAccesorioAuto").val()));
...
$(document).ready(function(){
  const urlParams = new URLSearchParams(window.location.search);
    ...
    $("#idAccesorioAuto").val(id);
    $("#idAccesorioAutoInv").val(id);
    $.getJSON('./index.php?action=producto/accesorios-autos-controller&method=edit&id='+id, function(v) {

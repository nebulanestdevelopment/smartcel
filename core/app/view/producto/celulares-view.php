<div class="card mb-4 mt-1">
            <div class="card-body">
              <div class="d-flex justify-content-between">
                <div>
                  <h4 class="card-title mb-0">Lista de celulares</h4>
                </div>
                <div class="btn-toolbar d-none d-md-block" role="toolbar" aria-label="Toolbar with buttons">
                    <button type="button" class="btn btn-outline-dark" onclick="window.location.href = './index.php?view=producto/celulares-add';">Agregar Equipo Celular nuevo</button>
                </div>
              </div>
...
<?php if(Roles::hasAdmin()){ ?>
  ...
  table += '<th>Celular</th><th>IMEI #1</th><th>IMEI #2</th><th>Categoria</th>';
  table += '<th>Costo</th><th>Precio</th><th>Precio Especial</th><th class="text-center">Qty</th>';
  table += '<th>Fecha</th><th>Proveedor</th><th>Acciones</th><th>Historial</th></tr></thead>';
  table += '<tbody></tbody></table></div>';
function drawTable(){
  $("#table-repuesto").empty().append(table); 
  new DataTable('#example', {
                ajax: "./index.php?action=producto/celulares-controller&method=get-all",
                columns: [  
        { data: null,
                render: function(data,type){
                   return data.marca+' '+data.modelo+' '+data.color+' '+data.categoria;           
                }
        },
        { data: 'imei1' },
        { data: 'imei2' },
       ...
       { data: null,
        ...
        { data: null,
                render: function(data,type){
                  return  '<a href="./index.php?view=historial/celulares&id='+data.id+'" class="btn btn-link" style="text-decoration-line: none;">'+data.unidades+'</a>';
                }
        },
        { data: 'fecha' },
        { data: 'proveedor' },        
...
<?php }else{ ?>
  ...
  table += '<thead class="table-dark"><tr>';
  table += '<th>Celular</th><th>IMEI #1</th><th>IMEI #2</th><th>Categoria</th>';
  table += '<th>Precio</th><th>Precio Especial</th><th class="text-center">Qty</th>';
  table += '<th>Fecha</th></tr></thead>';
  table += '<tbody></tbody></table></div>';
function drawTable(){
  $("#table-repuesto").empty().append(table); 
  new DataTable('#example', {
        ...
        createdRow: (row, data, index) => {
          row.querySelector(':nth-child(7)').classList.add('text-center');
            if (parseInt(data.unidades) === 0) {
            $(row).css("background", "#ebcccc");
            }
        }, 
...
function editarRepuesto(id){
  window.location = './index.php?view=producto/celulares-edit&id='+id;
}

function deleteRepuesto(id){
  if (confirm('Desea eliminar el celular') == true) {
      var request;
        if (request) {
          request.abort();
      }

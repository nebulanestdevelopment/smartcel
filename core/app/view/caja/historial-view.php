...
<div class="card mb-4 mt-1">
            <div class="card-body">
              <div class="d-flex justify-content-between">
                <div>
                  <h3 class="card-title mb-0"><svg class="icon" style="width: 20px;height: 20px;"><use xlink:href="vendors/@coreui/icons/svg/free.svg#cil-3d"></use></svg> Caja</h3>
                </div>
              </div>
              <div class="d-flex justify-content-end">
                <div class="btn-group" role="group" aria-label="Basic outlined example">
                  <button type="button" class="btn btn-outline-primary" onclick="window.location.href = './index.php?view=caja/historial-caja';">Historial</button>
                  
                  <?php
                    if(count($data) > 0){?>
                       <button type="button" class="btn btn-outline-primary" onclick="window.location.href = './index.php?action=caja/procesar';">Procesar Venta</button>
                  <?php  }
                  ?>
                </div>
              </div>
...
<table id="get-all" class="table table-striped table-bordered table-hover table-sm" >
    ...
    <?php
     ...
     $revirtiendo = false;     
     $revertir = 0;
     $id_caja_a_revertir = 0;
        foreach($data as $v){
          if(intval($v['id_revertir']) != 0){
            $revirtiendo = true;
            $revertir = 1;
            $id_caja_a_revertir = $v['id_revertir'];
       }
...
<?php  if($revertir == 1) : ?>
<h5 class="alert alert-danger" style="font-size: 16px;text-transform: capitalize;">Agregando ventas a caja, estas revirtiendo caja
&nbsp; <a href="./index.php?action=caja/revertir&id=<?php echo $id_caja_a_revertir;?>" class="btn btn-danger ">Actualizar caja <i class="fa fa-arrow-right"></i></a>
</h5>
<?php endif;?>
...
$('#table-ventas-total-r').css('display', 'block').css('text-align', 'center').empty().append('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" style="width:500px;"><radialGradient id="a5" cx=".66" fx=".66" cy=".3125" fy=".3125" gradientTransform="scale(1.5)"><stop offset="0" stop-color="#FF156D"></stop><stop offset=".3" stop-color="#FF156D" stop-opacity=".9"></stop><stop offset=".6" stop-color="#FF156D" stop-opacity=".6"></stop><stop offset=".8" stop-color="#FF156D" stop-opacity=".3"></stop><stop offset="1" stop-color="#FF156D" stop-opacity="0"></stop></radialGradient><circle transform-origin="center" fill="none" stroke="url(#a5)" stroke-width="15" stroke-linecap="round" stroke-dasharray="200 1000" stroke-dashoffset="0" cx="100" cy="100" r="70"><animateTransform type="rotate" attributeName="transform" calcMode="spline" dur="2" values="360;0" keyTimes="0;1" keySplines="0 0 1 1" repeatCount="indefinite"></animateTransform></circle><circle transform-origin="center" fill="none" opacity=".2" stroke="#FF156D" stroke-width="15" stroke-linecap="round" cx="100" cy="100" r="70"></circle></svg>');
      ...
      $.ajax({
        url: './index.php?action=ventas/get-all-sells-type-caja',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        ...
        error: function(jqXHR, textStatus, errorThrown) {
          $("#btn-verr").html('Ver resultados');
          $("#btn-verr").attr('id','btn-ver-r');
        }
      });
});

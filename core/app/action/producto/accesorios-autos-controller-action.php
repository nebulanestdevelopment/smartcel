...
if(isset($_GET['method']) && $_GET['method'] == "get-all"){
    $data = (new AccesoriosAutosData)->getAll();
    ...
    if(intval($_GET['category']) == 0){
        ...
        $data = (new AccesoriosAutosData)->getAllByCategory($_GET['category']);
    }
    echo json_encode(["data" => $data]);
    exit;
    ...
    $data = (new AccesoriosAutosData)->getById($_GET['id']);
   ...
   Roles::validHasAdmin();
    ...
    $id=filter_var($_POST["id"], FILTER_VALIDATE_INT);
    $resultado = (new AccesoriosAutosData)->duplicar($id);
    $code = (!$resultado['code'])?400:201;
    http_response_code($code);

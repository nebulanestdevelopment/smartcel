<?php
class AccesoriosData extends ProductoData {
    ...
    public static function getAll() {
            ...
            WHERE p.type=1 Order BY p.id ASC;";
            ExecutorPDO::initCon();
    ...
    public static function getAllByCategory($id) {
        ...
        LEFT OUTER JOIN person pe ON p.person_id = pe.id 
        LEFT OUTER JOIN category c ON p.category_id = c.id
        WHERE p.type=1 AND p.category_id='".$id."' Order BY p.id ASC;";
        ExecutorPDO::initCon();
        ExecutorPDO::initPreparate($sql);
        $array = array();
        ExecutorPDO::executeParams(array());
...
public function duplicar($id){
    ...
    try {
        ...
        $stmt->execute();
        $p = $stmt->fetchAll(PDO::FETCH_CLASS,"AccesoriosData");
        $p = $p[0];

    $lastCode = intval(ProductoData::getLastCode(1)['code']) + 1;

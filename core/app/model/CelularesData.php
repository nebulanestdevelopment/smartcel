<?php
class CelularesData extends ProductoData {

    public static function getAll() {
            $sql = "SELECT p.id,p.barcode,p.imei2,p.image,p.codigo,p.name,p.description,p.price_in,p.price_in_d,p.price_out,p.price_out_d,p.price_out_es_d,p.price_out_es,p.presentation,p.marca,p.modelo,p.color,
            ...
            WHERE p.type=2 Order BY p.id ASC;";
            ExecutorPDO::initCon();
            ...
            ExecutorPDO::executeParams(array());
                foreach  (ExecutorPDO::fetchAll()  as $row) {
                    ...
                    if(intval($units) == 0){ continue;}
                    $array[$cnt]['description'] = $row['description'];
                    $array[$cnt]['id'] = $row['id'];
                    $array[$cnt]['name'] = $row['name'];
                    $array[$cnt]['image'] = $row['image'];
                    $array[$cnt]['imei1'] = $row['barcode'];
                    $array[$cnt]['imei2'] = $row['imei2'];
                    $array[$cnt]['codigo'] = $row['codigo'];
                    $array[$cnt]['costo_c'] = floatval($row['price_in']);
                    $array[$cnt]['costo_d'] = floatval($row['price_in_d']);
                    $array[$cnt]['precio_c'] = floatval($row['price_out']);
                    $array[$cnt]['precio_d'] = floatval($row['price_out_d']);
                    $array[$cnt]['precio_es_c'] = floatval($row['price_out_es']);
                    $array[$cnt]['precio_es_d'] = floatval($row['price_out_es_d']);
                    $array[$cnt]['unidades'] = $units;
                    $array[$cnt]['presentation'] = $row['presentation'];
                    $array[$cnt]['proveedor'] =  $row['proveedor'];
                    $array[$cnt]['marca'] = $row['marca'];
                    $array[$cnt]['modelo'] = $row['modelo'];
                    $array[$cnt]['color'] = $row['color'];
                    $array[$cnt]['categoria'] = $row['categoria'];
                    $array[$cnt]['fecha'] = date('d/m/Y', strtotime($row['created_at']));
                    $array[$cnt]['is_admin'] = Roles::hasAdmin() ? 1 : 0 ;
                    $cnt++;
                }
    ...
    public static function getById($id) {
        $sql = "SELECT p.id,p.image,p.codigo,p.barcode,p.imei2,p.name,p.description,p.price_in,p.price_in_d,p.price_out,p.price_out_d,p.price_out_es_d,p.price_out_es,p.presentation,p.marca,p.modelo,p.color,
...
public static function getMovimientosById($id) {
    $sql = "SELECT o.id, o.product_id, o.q, o.price_in,o.price_in_d,o.price_out,o.price_out_d,o.price_out_es,o.price_out_es_d, o.discount, 
            ...
            CONCAT(p.marca,' ',p.modelo,' ',p.color,' ',c.name) AS item,o.descripcion                     
            FROM operation o 
...
public function save(){
    ...
    try {
            $sql_producto = "INSERT INTO product (`image`,`barcode`, `codigo`, `imei2`, `imeivarios`, `type`,
                ...
                if(!$producto){
                   throw new PDOException("No se pudo guardar el celular");
                }
                ...
                $compra_d = floatval(floatval($this->price_in_d) * $this->unit);
                    ...
                    $sql_sell = "INSERT INTO `sell` (`person_id`, `user_id`, `operation_type_id`,`id_caja_a_revertir`, 
                        ...
                        return ["msg"=>"Celular registrado correctamente...","code"=>true];      
    ...
    try {
            $sql_producto = "INSERT INTO product (`image`,`barcode`, `codigo`, `imei2`, `imeivarios`, `type`,
                ...
                $idp = $con->lastInsertId(); 
                        ...
                        $con = null;
                        ExecutorPDO::closeConnection();                    
                        return ["msg"=>"Celular registrado correctamente...","code"=>true,'id'=>$idp];      
